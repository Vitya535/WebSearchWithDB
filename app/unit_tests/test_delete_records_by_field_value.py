...
class DeleteRecordByFieldValueTestCase(CommonSettingsForTestCase):
    ...
    def test_delete_watch_history_record_by_watch_time(self):
        ...
        watch_history_record_for_delete = DB.session.query(WatchHistoryRecord) \
            .filter_by(watch_time=datetime.now().strftime("%Y-%m-%d %H:%M:%S")) \
            .first()
        delete_watch_history_record_by_watch_time(datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
        watch_history_records = DB.session.query(WatchHistoryRecord).all()
        self.assertEqual(len(watch_history_records), RECORDS_COUNT_WATCH_HISTORY - 1)
    ...
    def test_delete_search_history_record_by_search_query(self):
        ...
        self.assertEqual(len(search_history_records), RECORDS_COUNT_SEARCH_HISTORY - 1)
